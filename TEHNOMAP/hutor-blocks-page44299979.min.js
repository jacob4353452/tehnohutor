...
function t481_setWidth(recid) {
  ...
  Array.prototype.forEach.call(menuBlocks, function (menu) {
    ...
    var centerContainerList = centerContainer
      ? centerContainer.querySelectorAll("li")
      : [];
    var maxWidth = Math.max(
      t481_getFullWidth(leftContainer),
      t481_getFullWidth(rightContainer)
    );
...
function t481_getFullWidth(element) {
  if (!element) return 0;
  var marginLeft =
    element.style.marginLeft || window.getComputedStyle(element).marginLeft;
  var marginRight =
    element.style.marginRight || window.getComputedStyle(element).marginRight;
